...
function compileRulesContent({
  ...
  function stripMemoryMetadata(fact: string): string[] {
    // Facts may be stored as a flat string (no real \n) or as multiline text.
    // Normalise: split on ## section markers so each becomes its own block,
    // then strip only the leading # Title and *Generated footer.
        ...
        <div className="flex flex-col gap-5 py-2">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-2">
              <label className={`flex items-start gap-2.5 p-3 rounded-lg border cursor-pointer transition-all bg-surface hover:bg-surface2 ${
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              id="target-format"
              value={targetFile}
              onChange={(e: any) => setTargetFile(e.target.value)}
            >
        ...
        <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-4 py-2 my-2 no-scrollbar">
          ...
          {!isLoading && !isError && versions.map((v: any, index: number) => {
            ...
            return (
              <div
                key={v.id}
                className="relative border-l-2 border-accent pl-5 pb-3 mb-2 last:mb-0"
              >
