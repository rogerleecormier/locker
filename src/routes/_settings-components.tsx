...
function WebhookSecretRow({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getWebhookSecret({ data: { source, scopeKey } }),
  });
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      await setWebhookSecret({ data: { source, secret: inputValue.trim(), scopeKey } });
      qc.invalidateQueries({ queryKey: ["webhook-secret", source, scopeKey] });
      setEditing(false);
      setInputValue("");
    } catch (e: unknown) {
  ...
  async function handleDelete() {
    if (!confirm(`Remove ${label} webhook secret? This will break any active webhooks until a new secret is configured.`)) return;
    try {
      await deleteWebhookSecret({ data: { source, scopeKey } });
      qc.invalidateQueries({ queryKey: ["webhook-secret", source, scopeKey] });
    } catch (e: unknown) {
  ...
  return (
    <div className="border border-border rounded-xl bg-surface2 overflow-hidden">
      <div className="flex items-center justify-between p-4 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${color}22`, border: `1px solid ${color}44` }}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isLoading ? (
            ...
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-surface text-text-muted border border-border">
              Not set
            </span>
          )}
          {configured && !editing && (
            <Button variant="ghost" size="sm" onClick={handleDelete} className="text-[11px] text-red-400 hover:text-red-300 px-2">
              Remove
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => { setEditing((v) => !v); setInputValue(""); setError(null); }} className="text-[11px] px-3">
        ...
        <div className="border-t border-border px-4 py-3 flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <Input
              ...
              autoFocus
            />
            <Button size="sm" onClick={handleSave} disabled={saving || !inputValue.trim()} className="px-4">
              {saving ? "Saving…" : "Save"}
            </Button>
          </div>
...
export function WebhookSecretsSection() {
  ...
  return (
    <div className="flex flex-col gap-6 py-2">
      <div>
        <h2 className="text-base font-bold text-text mb-1">Webhook Integrations</h2>
        <p className="text-xs text-text-muted leading-relaxed max-w-2xl">
          Configure signing secrets to enable automatic memory commits when a GitHub PR is merged
          or a Linear ticket is completed. Each scope (personal, org) stores its own secret — team
          members inherit the secret from their parent org.
        </p>
      </div>

      {displayScopes.map((ws) => (
        <div key={ws.key} className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-bold text-text-muted uppercase tracking-wider">{ws.label}</span>
