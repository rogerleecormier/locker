...
// ── Landing page interactive playground ──────────────────────────────────────
...
const demoPlayground = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ result: string }> => {
    // Simple echo-style playground — no real AI needed; just illustrate the UX.
    if (data.action === "add") {
          ...
          <div ref={logRef} style={{ flex: 1, padding: 12, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6, minHeight: 160, maxHeight: 200 }}>
            ...
            {log.map((l, i) => (
              <div key={i} style={{
                ...
                color: l.type === "in" ? "var(--accent)" : "var(--text-muted)",
                paddingLeft: l.type === "out" ? 14 : 0,
              }}>
                {l.type === "in" ? "$ " : "→ "}{l.text}
              </div>
            ))}
