...
export function NewMemoryModal({ isOpen, onClose, onSaved, projectKey }: NewMemoryModalProps) {
  ...
  const [cliFormat, setCliFormat] = React.useState<"claude" | "cursor" | "copilot" | "gemini" | "agents">("cursor");
      ...
      <DialogContent className={cn("duration-300", modalWidthClass)}>
        ...
        {step === 1 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            ...
            >
              <span className="text-2xl group-hover:scale-110 duration-200 self-start">⚡</span>
              <span className="font-bold text-sm text-text">AI Tech Stack Creator</span>
              <span className="text-xs text-text-muted leading-relaxed">
        ...
        {step === 4 && recommendation && (
          <div className="flex flex-col gap-4 py-2 text-center items-center">
            ...
            <h3 className="text-base font-bold text-text">Tech Stack Vault Committed!</h3>
            <p className="text-xs text-text-muted leading-relaxed max-w-sm">
              Your architectural constraints have been committed to D1 and synced globally. You can now compile the local configuration file for your development agent.
            </p>
            ...
            <div className="w-full bg-surface2 border border-border rounded-xl p-4 mt-2 flex flex-col gap-3 text-left">
              <div className="flex justify-between items-center">
                ...
                >
                  ...
                  <option value="claude">.claudemd</option>
                  <option value="copilot">github-copilot</option>
                  <option value="gemini">.geminirules</option>
                  <option value="agents">agents.md</option>
                </Select>
              </div>
              <Button onClick={() => handleDownloadConfig(cliFormat)} className="w-full">
                Download {cliFormat === "cursor" ? ".cursorrules" : cliFormat === "claude" ? ".claudemd" : cliFormat === "gemini" ? ".geminirules" : cliFormat} File
              </Button>
            </div>
