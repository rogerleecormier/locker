...
export const compareImportedMemories = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ImportComparisonItem[]> => {
    ...
    const candidatesToCheck: {
      ...
      fact: string;
      category: string;
      potentialMatches: { id: string; fact: string; category: string }[];
    }[] = [];
    ...
    for (let i = 0; i < valid.length; i++) {
      ...
      if (itemMatches.length > 0) {
        candidatesToCheck.push({
          ...
          potentialMatches: itemMatches,
        });
      } else {
        comparisonResults[tempId] = {
          status: "new",
          matchedId: null,
          reason: "No semantically similar memories found in the database.",
        };
      }
    }
...
- "update": refines, expands, corrects, or updates the information in an existing memory (e.g. newer job, new location, updated preference).
- "contradiction": directly conflicts with an existing memory (e.g. "is single" vs "is married").
...
`;
          ...
          for (const item of batch) {
            if (!comparisonResults[item.tempId]) {
              comparisonResults[item.tempId] = {
                status: "new",
                matchedId: null,
                reason: "AI analysis failed, defaulting to new entry.",
              };
            }
          }
        }
    ...
    const output: ImportComparisonItem[] = valid.map((item, i) => {
      const tempId = `new-${i}`;
      const classification = comparisonResults[tempId] || {
        status: "new" as const,
        matchedId: null,
        reason: "No comparison could be completed.",
      };
        ...
        chunk.map(async (item) => {
          ...
          if (action === "archive_and_import" && matchedMemoryId) {
            ...
            if (existingRows.length > 0) {
              ...
              if (!existing.isLocked) {
                ...
                try {
                  await env.VECTOR_INDEX.deleteByIds([matchedMemoryId]);
                } catch (err) {
                  console.error("[executeImportActions] Vectorize delete failed:", err);
                }
          ...
          if (action === "update" && matchedMemoryId) {
            ...
            if (existingRows.length > 0) {
              const existing = existingRows[0];
              if (existing.isLocked) {
                throw new Error("Cannot update a locked memory.");
              }
              ...
              await env.VECTOR_INDEX.upsert([{
                id: matchedMemoryId,
                values: embedding,
                metadata: { userId: user.id, category, tags: finalTags, projectKey: existing.projectKey ?? "" } as Record<string, VectorizeVectorMetadata>,
              }]);
          ...
          if (action === "import" || action === "archive_and_import") {
            ...
            try {
              entityIds = await persistGraphData(env.DB, id, user.id, itemPk, graphExtraction);
            } catch (err) {
              console.error("[executeImportActions] graph persist failed:", err);
            }
            ...
            try {
              ...
              console.error(`[executeImportActions] vector insert failed:`, err);
            }
